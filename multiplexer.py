...
class Multiplexer:
    def __init__(self, template, payloads, cpu_count, savefile="~/.multiplexer_progress"):
        ...
        self.tasks = self.make_tasks(self.template, self.payload)
        self.completed_tasks = []

    def make_tasks(self, templates, payloads):
        """
    ...
    def resume(self):
        ...
        :return: None
        """
        self.tasks = open(self.savefile, 'r').read().splitlines()
    ...
    def spawn_process(self, command):
        ...
        retcode = subprocess.run(command.split(' '))
...
if __name__ == '__main__':
    ...
    payloads = read_into_list(args.payloads)
    ...
    mp = Multiplexer(commands, payloads, savefile=args.savefile)
